...
for tc in range(1, T + 1):
    ...
    while cnt != len(num_l)//2:
        customer2 = num_l[x][1:]
        monster2 = num_l[len(num_l)-x-1][1:]
        walk_cnt2 += abs(walk[0] - monster[0]) + abs(walk[1] - monster[1])
        walk_cnt2 += abs(monster[0] - customer[0]) + abs(monster[1] - customer[1])

        walk2 = customer2
        cnt2 += 1
        x2 +=1
    print(f'#{tc} {walk_cnt2}')
